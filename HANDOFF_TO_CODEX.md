...
**图都齐了，不用你补**。19 件物品全部用自己的图，没有一件是借的：
永久用 `luck:<id>`，一次性用 `tool:<id>`。三张原本画给旧道具的图跟着东西搬了位置：
`items[0]` 跑鞋 → `luck:shoes`、`extra4[4]` 耳塞 → `luck:earplug`、`main[23]` 笔电 → `luck:laptop`。

有两个名字换了，图还合用，但你要重画也可以：
- `luck:socks` 现在叫「软底舒服袜」（走路省动力）—— 袜子的图刚刚好
- `luck:potion` 现在叫「阿嬷的补药水」（每餐多回35%动力）—— 图上是四叶草药水，
  意思偏「幸运」多过「补身」，要换成一瓶补药/凉茶更贴切

如果以后要加新物品（例如保温饭盒、雨伞、二手脚踏车），再叫我开单给你。
规矩不变：图上不要写字，不要国旗，沿用原来的色盘。

## 2026-09-22 — 七个月、房租回家交、门口对准了

游戏规则改了一轮，art 这边**不用改**，只是几个名字换了，卡面上本来就没写字，所以不受影响。

**七个月**（本来12个月）。封面徽章已经改成「7 个月 · 别破产」。

**每个月必做三件事**：工作、吃饭、ROI。做不完回家也睡不着（动力耗尽还是会当场倒在路边）。
娱乐变成可选的，而且一个月只能去一次；一次性物品一个月也只能买一件。

**房租回「你的小窝」交**，房东在门口等，**只收钱包里的现金**，银行的钱不算。
银行利息改成每月随机 3%～9%，所以「存多少进银行」变成每个月都要想一次的事。

**回报研究所 → ROI研究所**。投资金额改成自己填，不管哪一种项目上限都是 $1000。
最后一个月投的钱会在月底结算时直接进银行，不会烂在里面。

**公告栏的股票内幕现在讲两个月**：这个月和下个月。两个月的涨跌都是月初就 roll 好的，
`scripts/check-stock-tips.js` 会验证工友说的和月底发生的一模一样。

**小偷**慢了一档（早上 0.58 倍、下午 0.66、晚上 0.74 的玩家速度）。第3月出现一个，
第6月多一个，多出来的那个只有 0.45 倍速，撞到才会被抢。

**门口对准了真实的门**：每栋楼多了一个 `doorX`，是从 town-map-v2.png 上量出来的，
不再假设门在建筑正中间（食堂的门在侧边、游戏厅的门偏左、杂货铺的门在左半边）。
互动距离从 46px 收到 36px，所以提示出现时你是真的站在门口。

**广场的长椅、花盆、告示板不再挡路**：`scripts/build-road-mask.py` 多了一条规则，
被地砖完全围住、小于 45 格的实心小岛当作街道家具放行。建筑本体永远碰得到绿化或框景，
所以不会被误放。重跑遮罩后 `node scripts/check-layout.js` 仍然 PASS。

## 2026-09-23 — 规则微调（art 完全不受影响）

上一节有几条已经过时了，以下为准：

- **一次性物品：每「种」一个月一次**，不是一个月只能买一件。不同种类可以一起买，
  背包还是 3 格（喷雾不占格）。
- **买卖股票、投资 ROI 都不扣动力**了。还会扣动力的只剩：走路、上班、兼职、杂货铺。
- **交易所五只股票全部上架**（本来每月随机 3 只）。这条是配合两个月内幕的 ——
  以前工友给的内幕可能是一只交易所根本没上架的股票。
- **银行画面不再提房租**，改成把「本月利息 X%」当标题。房租只在「你的小窝」讲。
- **免费餐券 $130 → $50**（一餐才 $70），**巴士票 $70 → $50**（本来比咖啡又贵又差）。
- **结局分级重写**：负数是「欠了一屁股债」，$0–999 是「月光光」，
  然后 $1k/$3k/$6k/$10k 分别是幸存者／银行常客／投资勇者／钱包战神。
- **小偷补位加快**：进店会清空街道，出来后第一个还是等 20 秒，第二个只等 7 秒。
  数量本来就对（第6月起两个），只是补得太慢，玩家常常整个月只见到一个。
- **卡面图框改成正方形**。美术图每一格都是正方形，之前被塞进 134×76 的框里，
  横向拉伸 1.76 倍，人物全变形了。现在手机 164×164、电脑 181×181，不再拉伸。
  `scripts/check-card-shape.js` 会盯着这件事，以后谁把框改成非正方形都会报错。

测试脚本（都要先开 `node scripts/preview-server.js`，端口改成 8934）：
`check-layout` `check-stock-tips` `check-endings`（这三个不用浏览器）、
`check-month-rules` `check-market` `check-thieves` `check-low-energy` `check-card-shape`。
